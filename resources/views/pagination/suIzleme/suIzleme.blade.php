            ...
            <div class="card-header">
                <div class="flex flex-col sm:flex-row items-center justify-between space-y-2 sm:space-y-0 sm:space-x-4">
                    <div class="flex space-x-2 w-full sm:w-auto">
                        ...
                        <button id="company" class="btn bg-primary text-white bg-blue-500 hover:bg-blue-600 w-full sm:w-auto text-center" data-fc-target="default-modal" data-fc-type="modal">
    ...
    <div class="fc-modal-open:opacity-100 duration-500 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto flex flex-col bg-white border shadow-sm rounded-md dark:bg-slate-800 dark:border-gray-700">
        ...
        <div class="px-4 py-8 overflow-y-auto max-h-96"> <!-- max-h-96 yüksekliği kontrol eder -->
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead>
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Firma</th>
                        <th scope="col" class="px-6 py-3 text-end text-xs font-medium text-gray-500 uppercase">İşlemler</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="odd:bg-white even:bg-gray-100 hover:bg-gray-100 dark:odd:bg-gray-800 dark:even:bg-gray-700 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $item->companyName }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                        <form method="POST" action="{{ route('pagination.suIzleme.CompDelete') }}">
                            @csrf
                            <input type="hidden" name="id" value="{{ $item->id }}">
                            <button type="submit" class="text-primary hover:text-sky-700 bg-transparent border-0 p-0">
                                Sil
                            </button>
                        </form>
               </td>
        ...
        <div class="flex justify-end items-center gap-4 p-4 border-t dark:border-slate-700">
            <button class="btn dark:text-gray-200 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 hover:dark:bg-slate-700 transition-all" data-fc-dismiss type="button">Çıkış</button>
        </div>
    </div>
</div>
@endsection
                ...
                const grid = new gridjs.Grid({
...

                    {
                        ...
                        formatter: (function (cell) {
                            ...
                            return gridjs.html("<a href='" + routeUrl + "' class='me-0.5'> <i class='mgc_eye_line text-lg'></i> </a>");
